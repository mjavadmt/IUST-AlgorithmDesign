...
namespace E1
{
    public class Q2RoadReconstruction : Processor
    {
        ...
        public long[][] Solve(long n, long[][] distance)
        {
            ...
            {
                if (BFS(adjacent_list_weights , edge.Item1 , edge.Item2) > distance[edge.Item1][edge.Item2])
                {
        ...
        private static long BFS(List<List<(long, long)>> adjacent_list, long startnode, long endnode)
        {
            ...
            while (vertexes.Any())
            {
                ...
                foreach (var item in adjacent_list[(int)dequeued])
                {
                    if (distance[item.Item1] == int.MaxValue)
                    {
                        ...
                        distance[item.Item1] = distance[dequeued] + (int)item.Item2;
                        // pre[item] = dequeued ;
                    }
                    if (item.Item1 == endnode)
                    {
                        return distance[item.Item1];
                    }
                }
            }
            return 0 ;
